...
export default function VeterinariosPage() {
    ...
    const eliminar = async (id) => {
        if (!window.confirm('¿Estás seguro de eliminar este veterinario?')) return
        try {
        ...
        <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
            ...
            <div style={{ padding: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    ...
                    {isAdmin() && (
                        <button onClick={() => navigate('/veterinarios/nuevo')} style={btnPrimary}>
                            + Nuevo veterinario
                        </button>
                    )}
                </div>
                                ...
                                {isAdmin() && (
                                    <td style={td}>
                                        <div style={{ display: 'flex', gap: '8px' }}>
                                            <button onClick={() => navigate(`/veterinarios/${v.id}/editar`)} style={btnEdit}>
                                                Editar
                                            </button>
                                            <button onClick={() => eliminar(v.id)} style={btnDelete}>
                                                Eliminar
                                            </button>
                                        </div>
                                    </td>
